...
vector<vector<int>> makeReducable(vector<vector<int>> matrix,int row,int col){
...
void tspbnb(vector<vector<int>> matrix,int par = -1,int nextcity=1,int passedCost=0){
    if(visited.size() == N ){
        return;
    }
    if(par == -1){
    ...
    for(int rem = 1 ; rem <= N ; rem++){
        if(visited.find(rem) != visited.end())continue;
        ...
        cout<<"Reduced matrix for path from city "<<nextcity<<" to "<<rem<<" is as :\n";
        print2d(tmp);
        cout<<"with reduction cost = "<<passedCost<<"+"<<tmpcost-matrix[nextcity-1][rem-1]<<"+"<<matrix[nextcity-1][rem-1]<<" = "<<tmpcost+passedCost<<"\n\n";
    }
    visited.insert(costind.second);
    //cout<<costind.second<<"->";
    path.push_back(costind.second);
    tspbnb(makeReducable(matrix,nextcity,costind.second),nextcity,costind.second,costind.first+passedCost);
}
